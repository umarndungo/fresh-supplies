...
Generates realistic vehicle telemetry logs with storage exposure fields.
"""
...
def extract_location_from_coords(lat, lon) -> str:
    """Convert numeric coordinates into a cardinal location string."""
...
def generate_raw_telemetry(
    ...
    """Generate deterministic transit and storage telemetry records."""
    np.random.seed(seed)
    ...
    storage_age_hours = np.round(np.random.uniform(0.0, 168.0, num_records), 2)
    storage_temperatures = np.round(np.random.normal(22.0, 3.5, num_records), 2)
    storage_pressures = np.round(np.random.normal(30.0, 2.0, num_records), 2)
    ...
    df = pd.DataFrame({
        ...
        "Storage_Age_Hours": storage_age_hours,
        "Storage_Temperature_C": storage_temperatures,
        "Storage_Pressure_PSI": storage_pressures,
    ...
    df["location"] = df.apply(
        lambda row: extract_location_from_coords(row["latitude"], row["longitude"]),
        axis=1,
    )
